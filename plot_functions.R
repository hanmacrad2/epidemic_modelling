...
PLOT_MCMC_GRID <- function(sim_data, mcmc_output, 
  ...
  print(mcmc_plot_inputs$model_params$m1[[1]])
  print(max(m1_mcmc))
  a_lim =  max(mcmc_plot_inputs$model_params$m1[[1]], max(m1_mcmc))
  a_lim2 =  max(mcmc_plot_inputs$model_params$m1[[1]], m1_mean)
  
  #m2
  m2_mean = cumsum(m2_mcmc)/seq_along(m2_mcmc)
  b_lim = max(mcmc_plot_inputs$model_params$m2[[1]], max(m2_mcmc))
  b_lim2 = max(mcmc_plot_inputs$model_params$m2[[1]], m2_mean)
  
  #m3
  m3_mean = cumsum(m3_mcmc)/seq_along(m3_mcmc)
  m3_lim =  max(mcmc_plot_inputs$model_params$m3[[1]], max(m3_mcmc))
  m3_lim2 =  max(mcmc_plot_inputs$model_params$m3[[1]], m3_mean) 
  ...
  #i.Infections
  if(!FLAGS_LIST$DATA_AUG) inf_tite = paste0(mcmc_plot_inputs$seed_count, ', ', mcmc_plot_inputs$model_typeX, " Data, r0 = ", true_r0) # 'Day Infts, '
  else inf_tite = paste0(mcmc_plot_inputs$seed_count, ', ', mcmc_plot_inputs$model_typeX, " Data, r0 = ", true_r0, ", + Data Aug")
  
  #i.Infections
  sim_dataX = non_ss + ss
  plot.ts(sim_dataX, xlab = 'Time', ylab = 'Daily Infections count',
          main = inf_tite, 
  ...
  plot.ts(m1_mcmc, ylab = mcmc_plot_inputs$mod_par_names[1], ylim=c(0, a_lim),
          ...
          cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(h = mcmc_plot_inputs$model_params$m1[[1]], col = 'red', lwd = 2) #True = green
  
  plot.ts(m2_mcmc, ylab = 'm2', ylim=c(0, b_lim), 
          main = paste("MCMC", mcmc_plot_inputs$model_typeX, ":", mcmc_plot_inputs$mod_par_names[2], "prior:", m2_prior),
          cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(h = mcmc_plot_inputs$model_params$m2[[1]], col = 'blue', lwd = 2) #True = green
  
  plot.ts(m3_mcmc,  ylab = 'm3', ylim=c(0,m3_lim),
          main = paste("MCMC", mcmc_plot_inputs$model_typeX, ":", mcmc_plot_inputs$mod_par_names[3], "prior:", m3_prior),
          cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(h = mcmc_plot_inputs$model_params$m3[[1]], col = 'green', lwd = 2) #True = green
  ...
  plot(seq_along(m1_mean), m1_mean,
       ...
       main = paste(mcmc_plot_inputs$mod_par_names[1], "MCMC mean, True", mcmc_plot_inputs$mod_par_names[1], "=", mcmc_plot_inputs$model_params$m1[[1]]),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  #print(plot2)
  abline(h = mcmc_plot_inputs$model_params$m1[[1]], col = 'red', lwd = 2)
  
  #m2 mean
  plot(seq_along(m2_mean), m2_mean,
       ylim=c(0, b_lim),
       xlab = 'Time', ylab = 'm2',
       main = paste(mcmc_plot_inputs$mod_par_names[2], "MCMC mean, True", mcmc_plot_inputs$mod_par_names[2], "=", mcmc_plot_inputs$model_params$m2[[1]]),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  #print(plot2)
  abline(h = mcmc_plot_inputs$model_params$m2[[1]], col = 'blue', lwd = 2)
  
  #m3 Mean
  plot(seq_along(m3_mean), m3_mean,
       xlab = 'Time', ylab = 'm3', 
       main = paste(mcmc_plot_inputs$mod_par_names[3], "MCMC mean, True", mcmc_plot_inputs$mod_par_names[3], "=", mcmc_plot_inputs$model_params$m3[[1]]),
       ylim=c(0, m3_lim),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  abline(h = mcmc_plot_inputs$model_params$m3[[1]], col = 'green', lwd = 2)
  ...
  hist(m1_mcmc, freq = FALSE, breaks = 100,
       ...
       main = paste(mcmc_plot_inputs$mod_par_names[1], ", True", mcmc_plot_inputs$mod_par_names[1], "=", mcmc_plot_inputs$model_params$m1[[1]]), 
       xlim=c(0, a_lim),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  ...
  hist(m2_mcmc, freq = FALSE, breaks = 100,
       ...
       main = paste(mcmc_plot_inputs$mod_par_names[2], ", True", mcmc_plot_inputs$mod_par_names[2], "=", mcmc_plot_inputs$model_params$m2[[1]]), 
       xlim=c(0, b_lim),
       cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
  ...
  m1_mean_tail = round(mean(m1_mcmc[mcmc_plot_inputs$n_mcmc - data_10_pc:mcmc_plot_inputs$n_mcmc]), 2) 
  m2_mean_tail = round(mean(m2_mcmc[mcmc_plot_inputs$n_mcmc - data_10_pc:mcmc_plot_inputs$n_mcmc]), 2)
  m3_mean_tail = round(mean(m3_mcmc[mcmc_plot_inputs$n_mcmc - data_10_pc:mcmc_plot_inputs$n_mcmc]), 2)
  m4_mean_tail = round(mean(r0_mcmc[mcmc_plot_inputs$n_mcmc - data_10_pc:mcmc_plot_inputs$n_mcmc]), 2)
  ...
  if (FLAGS_LIST$RJMCMC){
    ...
    hist(m1_mcmc, freq = FALSE, breaks = 100,
         xlab = mcmc_plot_inputs$mod_par_names[1], #ylab = 'Density', 
         main = paste("m1, True m1 = ", mcmc_plot_inputs$model_params$m1[[1]]), 
         xlim=c(0, a_lim),
         cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5)
    abline(v = mcmc_plot_inputs$model_params$m1[[1]], col = 'red', lwd = 2)
    ...
    df_results <- data.frame(
      ...
      m1 = mcmc_plot_inputs$model_params$m1[[1]],
      a_mc = a_mcmc_mean,
      m2 = mcmc_plot_inputs$model_params$m2[[1]],
      b_mc = b_mcmc_mean,
      m3 = mcmc_plot_inputs$model_params$m3[[1]],
      c_mc = c_mcmc_mean,
      R0 = true_r0, 
      R0_mc = r0_mcmc_mean,
