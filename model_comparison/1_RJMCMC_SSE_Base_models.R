...
rjmcmc_sse_base_II <- function(data, n, sigma, model_params, gamma_prior, gamma_priors,
  ...
  for(i in 2:n) {
    ...
    if(!(is.na(log_accept_prob)) && log(runif(1)) < log_accept_prob) {
      ...
      log_like_vec[i] = logl_new
    } else {
      alpha_vec[i] <- alpha_vec[i-1]
      log_like_vec[i] = log_like_vec[i-1]
    }
    #Alpha check
    ...
    if (beta_vec[i-1] > 0){ 
      ...
      log_accept_prob = logl_new - log_like_vec[i] #logl_prev
    ...
    if ((beta_vec[i] > 0) | (gamma_vec[i] > 0)){ #Look to it 
      ...
      log_accept_prob = logl_new - log_like_vec[i] - alpha_dash + alpha_vec[i]
...
rjmcmc_sse_base_v0 <- function(data, n, sigma, model_params, gamma_prior, gamma_priors,
                               x0 = 1, prior = TRUE, alpha_transform = FALSE) {#thinning_factor, burn_in
...
rjmcmc_sse_base <- function(data, n, sigma, model_params, gamma_prior, gamma_priors,
  ...
  for(i in 2:n) {
    ...
    if ((beta > 0) | (gamma > 0)){ #Look to it 
      ...
      #alpha
      if (alpha_transform) { #ro = alpha + beta*gamma. alpha_dash_base (R0_base) = alpha_sse + beta_sse*gamma_sse (R0_SSE)
        alpha_dash = alpha + beta*gamma #Increase. as alpha_dash is actually the new R_0. Encapsulates 'total R0'            #r0_current #r0_vec[i] #- beta_new*gamma_dash #Line added 
