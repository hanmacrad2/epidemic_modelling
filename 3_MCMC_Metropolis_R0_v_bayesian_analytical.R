...
MCMC_range_sd <- function(list_sd, data, n, r0_true){
  ...
  for (sigma in list_sd) {
    ...
    #i.MCMC chain
    pdf(paste("Results/", i, "_mcmc_chain_sd_", sigma, "_true_r0_", r0_true, ".pdf", sep="")) #, width=6, height=4)
    ...
    #ii.Cumulative Mean
    r0_mean = cumsum(r0_mcmc)/seq_along(r0_mcmc)
    plot2 = plot(seq_along(r0_mean), r0_mean, xlab = 'Time', ylab = 'R0', main = paste("Mean of R0 MCMC chain, True R0 = ",r0, ", sd of proposal = ", sigma))
    print(plot2)
    
    #iii. Histograms
    hist1 <- hist(r0_mcmc, breaks = 80, xlab = 'r0', ylab = 'Density', 
                  main = paste("Histogram of R0. True R0 = ",r0, ", sd of proposal = ", sigma))
    print(hist1)
    #iiib. Histogram - density
    hist1$counts <- hist1$counts/sum(hist1$counts)
    hist2 = plot(hist1, xlab = 'r0', ylab = 'Density', 
                 main = paste("Empirical density of r0. True R0 = ",r0, ", sd of proposal = ", sigma))
    print(hist2)
    dev.off()
