...
mcmc_ss_x4_prior <- function(data, n, sigma,  sigma_b, x0 = 1, prior = TRUE) { #burn_in = 2500
  ...
  for(i in 2:n) {
    ...
    #Store
    like_b[i] = exp(log_like_ss_lse(data, alpha_vec[i], beta_vec[i], gamma_vec[i]))
    prior_beta[i] = exp(-beta_vec[i])
...
plot_mcmc_x4_priors <- function(sim_data, mcmc_params, true_r0, dist_type, total_time, seed_count, prior){
  ...
  df_results <- data.frame(
    ...
    a_rte_b_g = round(mcmc_params[[14]],2),
    tot_time = total_time) 
...
sigma_b = 0.05
#prior = TRUE
mcmc_params = mcmc_ss_x4_prior(sim_data, n, sigma, sigma_b)
end_time = Sys.time()
