...
#Plot results 
plot_rep_results_color_code <- function(true_r0, model_type, sim_data_rep, df_sum_stats, list_p_vals){
  
  'Plot sim data, summary stats and true summary stat for a given mcmc rep' 
  #Setup
  par(mfrow = c(3,4))
  len_data = length(list_p_vals)
  colorsX <- rainbow(len_data*2+1)
  colors_line <- rainbow(c(45:45+len_data+1))
  ...
  for (i in c(1:len_data)){
    ...
    nrowX = nrow(df_sum_stats)
    X = df_sum_stats[1:nrow(df_sum_stats)-1,i]
    sum_stat_trim = upper_quantile(X, 0.99)
    
    #Hist - first half
    p1 = hist(sum_stat_trim[1:length(sum_stat_trim)/2], breaks = 100, #freq = FALSE, 
              #xlim = c(xmin, xmax),
    ...
    #Hist - second half
    p2 = hist(sum_stat_trim[length(sum_stat_trim)/2 + 1 : length(sum_stat_trim)], breaks = 100, #freq = FALSE, 
              #xlim = c(xmin, xmax),
              xlab = paste('', toupper(colnames(df_sum_stats)[i]), '< 99th quantile'),
              ylab = 'Num Samples',
              col = colorsX[i+2],
    ...
    plot(p1)
    plot(p2, add = T) #plot on same plot :) 
